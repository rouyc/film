...
require_once File::build_path(array('lib','Security.php'));
    ...
    class ModelUtilisateur extends Model
    {
        ...
        public static function getAllCompteByID($idCompte) {
            try{ $rep = Model::$pdo->prepare("SELECT * FROM Utilisateur U WHERE idUtilisateur = :idCompte");}
            ...
            $rep->execute(array('idCompte' => $idCompte));
        ...
        public static function getUtilisateurByEmail($email){
            try {
                $rep = Model::$pdo->prepare("SELECT * FROM Utilisateur
        ...
        public static function getBoolByEmail($email){
            try{ $rep = Model::$pdo->prepare("SELECT admin FROM Utilisateur U
        ...
        public static function addCompte($nomUtilisateur, $prenomUtilisateur, $dateNaissanceUtilisateur, $emailUtilisateur, $mdp, $mdp1, $adresseUtilisateur, $telephoneUtilisateur, $bool_admin)
        {
            ...
            if (self::estDansLaBDD($emailUtilisateur)) {
                if (strcmp($mdp, $mdp1) == 0) {
                 ...
                 Veuillez vous rendre sur ce lien afin de valider votre compte : http://webinfo.iutmontp.univ-montp2.fr/~becp/PHP/www/index.php?controller=utilisateur&action=validationMail ';
        ...
        public static function checkPassword($login,$mot_de_passe_chiffre){
                ...
                try {
                    $rep = Model::$pdo->prepare("SELECT mdp FROM Utilisateur
        ...
        public static function estValide($login) {
            try {
                $rep = Model::$pdo->prepare("SELECT Valide FROM Utilisateur
                                                WHERE emailUtilisateur=:login");
            } catch (PDOException $e) {
                if (Conf::getDebug()) {
                    echo $e->getMessage();
        ...
        public static function estAdmin($login) {
            try {
                $rep = Model::$pdo->prepare("SELECT admin FROM Utilisateur
                                                WHERE emailUtilisateur= :login");
            } catch (PDOException $e) {
                if (Conf::getDebug()) {
                    echo $e->getMessage();
        ...
        public static function validationCompte($login,$mdp) {
            $U = self::getUtilisateurByEmail($login);
            $mdp = str_replace(" ","",$mdp);
